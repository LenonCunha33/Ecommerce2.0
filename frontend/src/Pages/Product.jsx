...
const Product = () => {
  ...
  if (!product) {
    return <div className="h-96 flex items-center justify-center animate-pulse text-gray-400 text-xl">Carregando produto...</div>;
  }
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="pt-12 px-6 md:px-12 max-w-screen-xl mx-auto text-lg">
      <div className="flex flex-col lg:flex-row gap-14">
        ...
        <div className="w-full lg:w-1/2 space-y-8">
          ...
          <p className="text-4xl font-bold text-gray-900">{currency}{product.price}</p>
          {!!product.description && <p className="text-gray-600 text-[1.05rem] leading-relaxed">{product.description}</p>}

          <div className="flex items-center gap-3">
            <a href="#comprar" onClick={handleBuy} className="inline-flex items-center justify-center bg-black hover:bg-gray-800 text-white py-4 px-8 rounded-md shadow transition-colors text-lg">COMPRAR</a>
            <button onClick={() => toggleFavorite(product)} className={`rounded-md px-4 py-3 border ${fav ? "bg-red-600 text-white border-red-600" : "border-gray-300"}`}>
      ...
      <Modal open={openSizeModal} onClose={() => setOpenSizeModal(false)} title="Escolha o tamanho para continuar a compra">
        <div className="grid grid-cols-3 gap-2">
          {Object.entries(sizeLinks).length === 0 && <p className="col-span-3 text-sm text-gray-600">Nenhum link configurado para este produto.</p>}
          {Object.entries(sizeLinks).map(([tamanho]) => (
            <button key={tamanho} onClick={() => goToSizeLink(tamanho)} className="rounded-md border px-3 py-2 text-sm font-medium hover:bg-gray-50">
              {tamanho}
            </button>
          ))}
        </div>
